...
async function checkIfCompleted() {
  ...
  try {
    ...
    if (user.completedTopics && user.completedTopics.includes("polynomial_representation_unit2")) {
      setCompletedState();
    }
  } catch (err) {
...
async function markComplete() {
  ...
  try {
    const response = await fetch("https://idsnext-backend.onrender.com/api/users/complete-topic", {
      ...
      body: JSON.stringify({ topicId: "polynomial_representation_unit2" })
    });
