...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnBottom) {
    btnBottom.addEventListener("click", markComplete);
  }
...
async function checkIfCompleted() {
  ...
  try {
    ...
    if (user.completedTopics && user.completedTopics.includes("tree_representation_unit3")) {
      setCompletedState();
    }
...
async function markComplete() {
  ...
  try {
    const response = await fetch("https://idsnext-backend.onrender.com/api/users/complete-topic", {
      ...
      body: JSON.stringify({
        topicId: "tree_representation_unit3"
      })
    });
