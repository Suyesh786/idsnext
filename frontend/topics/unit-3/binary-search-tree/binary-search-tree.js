...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnBottom) btnBottom.addEventListener("click", markComplete);
...
async function checkIfCompleted() {
  ...
  try {
    ...
    if (user.completedTopics && user.completedTopics.includes("binary_search_tree_unit3")) {
      setCompletedState();
    }
  } catch (err) {
...
async function markComplete() {
  ...
  try {
    const response = await fetch("https://idsnext-backend.onrender.com/api/users/complete-topic", {
      ...
      body: JSON.stringify({ topicId: "binary_search_tree_unit3" })
    });
